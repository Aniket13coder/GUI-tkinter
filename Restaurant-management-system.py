...
class RestaurantOrderManagement:
    def __init__(self, root):
        ...
        self.exchange_rate = 95.36
        self.setup_background()  
        frame = ttk.Frame(root)
        frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
        ttk.Label(frame, text="Restaurant Order Management",
                  font=("Arial", 20, "bold")).grid(row=0, columnspan=3, pady=10)
        self.menu_labels = {}
        ...
        ttk.Label(frame, text="Currency:").grid(
            row=len(self.menu_items) + 1, column=0)
        ...
        ttk.Button(frame, text="Place Order",
                   command=self.place_order).grid(
            row=len(self.menu_items) + 2, columnspan=3, pady=10)
    def setup_background(self):
        try:
            img = Image.open("restaurant-bgimage.jpg")   # can be png/jpg
            img = img.resize((800, 600))
            self.bg = ImageTk.PhotoImage(img)
            label = tk.Label(self.root, image=self.bg)
